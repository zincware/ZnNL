...
class PointsOnLattice(DataGenerator, ABC):
    ...
    def build_pool(self, x_points: int = 10, y_points: int = 10, boundary: float = -1):
        """
        ...
        boundary : float (default = -1)
                Symmetric boundary of the lattice.
        ...
        if boundary == -1:
            x = onp.linspace(
                -x_points / 2, x_points / 2, x_points + 1, dtype=int
            ).astype(float)
            y = onp.linspace(
                -y_points / 2, y_points / 2, y_points + 1, dtype=int
            ).astype(float)
        else:
            x = onp.linspace(-boundary, boundary, x_points + 1, dtype=float)
            y = onp.linspace(-boundary, boundary, y_points + 1, dtype=float)
